...
const Giveaways = ({ $ }) => {
    ...
    const { data: _guild } = swr("https://api.lunyx.me/v1/guilds/" + id + "/check");
    const guild = _guild ? _guild.data : null;

    const { data: _stats } = swr("https://api.lunyx.me/v1/guilds/" + id + "/stats");
    const stats = _stats ? _stats.data : null;

    const { data: _giveaways } = swr("https://api.lunyx.me/v1/giveaway/list?id=" + id);
