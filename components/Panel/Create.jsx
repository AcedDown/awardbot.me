...
export default function Create({ isOpen, closeModal, mutateList }) {
    ...
    const addPartner = async () => {
        ...
        const _add = await axios.post(
            "https://api.lunyx.me/v1/partners/add?_token=" + 
        ...
        >
            ...
            >
                ...
                <div className="my-5">
                    ...
                    <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Lunyx" type="text" className="mb-2 text-white bg-black/10 py-2 px-4 rounded-xl focus:outline-none w-full border border-white/10 focus:border-amber-400 focus:text-amber-400 transform duration-200" spellCheck={false} autoComplete={false} />
                    <span className="text-xs text-zinc-400 ml-2">Description:</span>
                    <input value={desc} onChange={e => setDesc(e.target.value)} placeholder="This is a description to be displayed." type="text" className="mb-2 text-white bg-black/10 py-2 px-4 rounded-xl focus:outline-none w-full border border-white/10 focus:border-amber-400 focus:text-amber-400 transform duration-200" spellCheck={false} autoComplete={false} />
                    <span className="text-xs text-zinc-400 ml-2">Website URL:</span>
                    <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://lunyx.me" type="text" className="text-white bg-black/10 py-2 px-4 rounded-xl focus:outline-none w-full border border-white/10 focus:border-amber-400 focus:text-amber-400 transform duration-200" spellCheck={false} autoComplete={false} />
                </div>
